...
void weight_hist_loop_template(){
  ...
  TFile * trigger_BCDEF = new TFile("EfficienciesAndSF_BCDEF.root");
  
  TFile * trigger_GH = new TFile("EfficienciesAndSF_GH.root");
  double lumi_BCDEF = 18.8442;
  double lumi_GH = 15.4089;
  double lumi_tot = lumi_BCDEF + lumi_GH;
  TFile * trigger_allRuns = new TFile("MuonID_EfficienciesAndSF_average_RunBtoH.root", "RECREATE");
