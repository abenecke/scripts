...
void weight_hist_isolation(){
  ...
  TFile * trigger_BCDEF = new TFile("Iso_EfficienciesAndSF_BCDEF.root"); 
  TFile * trigger_GH = new TFile("Iso_EfficienciesAndSF_GH.root");

  double lumi_BCDEF = 18.8442;
  double lumi_GH = 15.4089;
  double lumi_tot = lumi_BCDEF + lumi_GH;
  TFile * trigger_allRuns = new TFile("MuonIso_EfficienciesAndSF_average_RunBtoH.root", "RECREATE");
