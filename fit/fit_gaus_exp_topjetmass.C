...
void fit_gaus_exp_topjetmass()
{
  ...
  TString hist_name = "topjet_chi2cut_W/mass";
  ...
  TF1* background_fit = new TF1("background_fit", " [0]* exp([1]*x)", 51, 225);
  background_fit->SetParameter(0, 5.72);
  background_fit->SetParameter(1, -0.033);
  ...
  /////////////////////////   Fit function (Signal)  ////////////////////////
  TF1 *signal_fit = new TF1("signal_fit","[0]*exp(-(x-[1])^2/(2*[2]^2))", 51,225);
  ...
  TF1 *signal_background_fit = new TF1("signal_background_fit","signal_fit + background_fit", 51,225);
  ...
  /////////////////////////   Fit function (Data)  ////////////////////////
  TF1* data_fit = new TF1("data_fit","signal_fit+background_fit" , 51, 225);
  ...
  TF1 *MC_Eff_f =new TF1("MC_Eff_f","signal_background_fit - background_fit", 51,225);
   //signal
  ...
  MC_Eff_f->SetParameter(5, background_fit->GetParameter(0));
  MC_Eff_f->SetParameter(6,background_fit->GetParameter(1));



  MC_Eff_f->Draw();

  TF1 *Data_Eff_f =new TF1("Data_Eff_f","data_fit - background_fit", 51,225);
  //signal
  ...
  // background only
  Data_Eff_f->SetParameter(5, background_fit->GetParameter(0));
  Data_Eff_f->SetParameter(6,background_fit->GetParameter(1));
  ...
  Double_t num_MC_before = MC_Eff_f->Integral(51,225);
  cout<<"MC number before tagger:   "<< num_MC_before <<endl;
  Double_t num_Data_before = Data_Eff_f->Integral(51,225);
  ...
  cout << "number of sig events:  " << number_sig <<endl;

  //count events in data histogram
  Double_t number_data=0;  
  for(int i =0 ; i<data->GetSize();i++){
    number_data = number_data + data->GetBinContent(i);
  }
  cout << "number of sig events:  " << number_sig <<endl;
  ...
  myfile<<"number of background events:  " << number_back <<endl;
  myfile<<"number of signal events:  " << number_sig <<endl;
 myfile<<"number of data events:  " << number_data <<endl;
  myfile.close();
