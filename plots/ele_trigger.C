...
TGraphAsymmErrors* get_eff(TString sample, TString obs, TString hname1,double &tot_eff, double &tot_err,TString hname_all,TString hname_trig )
{
  ...
  if (sample.CompareTo("data", TString::kIgnoreCase) == 0){
    file_tot = new TFile("/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/CMSSW_8X/PreSelection/rootfiles/none/elec/uhh2.AnalysisModuleRunner.Data.Data.root", "READ");
    file_pass = new TFile("/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/CMSSW_8X/rootfiles/elec_channel_trigger115_only/uhh2.AnalysisModuleRunner.Data.Data.root", "READ");
  } else if (sample.CompareTo("Background", TString::kIgnoreCase) == 0){
    file_tot = new TFile("/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/CMSSW_8X/PreSelection/rootfiles/none/elec/uhh2.AnalysisModuleRunner.MC.MC_ZPrime1500ToTPrime700T_TPrimeToHT.root", "READ");
    file_pass = new TFile("/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/CMSSW_8X/rootfiles/elec_channel_trigger115_only/uhh2.AnalysisModuleRunner.MC.MC_ZPrime1500ToTPrime700T_TPrimeToHT.root", "READ");
  } 
 		
  TString name_all = "trigger_electron/pt_1"; 
  TH1F* d_all = (TH1F*) file_pass->Get(name_all);
  TCanvas *all_c = new TCanvas("all_c","All",10,10,700,900);
  d_all->Draw();

  TString name_trig = "input_eLep/pt_1"; 
  TH1F* d_trig = (TH1F*) file_pass->Get(name_trig);
  ...
  if (sample.CompareTo("Background", TString::kIgnoreCase) == 0){
...
void draw_eff(TGraphAsymmErrors* eff, TString epsfilename, TString sample, double tot_eff, double tot_err)
{
  ...
  painter->SetTitle("Signal Sample (1.5GeV,0.7GeV)");
...
void draw_both_eff(TGraphAsymmErrors* eff, TString epsfilename, TString sample, double tot_eff, double tot_err,TGraphAsymmErrors* eff2, TString sample2, double tot_eff2, double tot_err2)
{
  ...
  painter->GetXaxis()->SetTitle("p_{T} electron");
  painter->GetYaxis()->SetTitle("Efficiency");
  painter->SetTitle("");
  painter->GetXaxis()->SetTitleSize(0.05);
  painter->GetYaxis()->SetTitleSize(0.05);
  ...
  //  painter->GetYaxis()->SetRangeUser(0, 0.004);
  ...
  TLegend *leg = new TLegend(0.5,0.48,0.75,0.28, NULL,"brNDC");
  leg->AddEntry(eff,"Signal Sample", "lpe");
  leg->AddEntry(eff2,"Data", "lpe");
  leg->SetBorderSize(0);
  leg->SetFillStyle(0);
  leg->Draw();
...
void ele_trigger()
{
  gStyle->SetOptFit(1111);
   ...
   TString snames[] = {"Data", "Background" };
  TString prefix = "";
  ...
  //   TGraphAsymmErrors* eff = get_eff(sample, obs, hist,  tot_eff, tot_err,hname_all,hname_trig);
    ...
    //  draw_eff(eff, "draw_both.eps", sample, tot_eff, tot_err);
 ...
 TString hname_all="topjet_topjet2/";
 TString hname_trig="topjet_pass_zwtag/";
 TString filename = "draw_both.eps";
 TGraphAsymmErrors* eff = get_eff(snames[1], obs, hist,  tot_eff, tot_err,hname_all,hname_trig);
 draw_eff(eff, "signal_eff.eps", "Background", tot_eff, tot_err);
 TGraphAsymmErrors* eff2 = get_eff(snames[0], obs, hist,  tot_eff2, tot_err2,hname_all,hname_trig);
 draw_eff(eff2, "data_eff.eps", "Data", tot_eff2, tot_err2);

 draw_both_eff( eff, "draw_both.eps", "Background",  tot_eff,  tot_err, eff2, "Data",  tot_eff2,  tot_err2);
