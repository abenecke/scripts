...
TString folder = "/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/CMSSW_8X/pictures/mistag/h1btag";
...
TGraphAsymmErrors* get_eff(TString sample, TString obs, TString hname1,double &tot_eff, double &tot_err,TString hname_all,TString hname_trig )
{
  ...
  if (sample.CompareTo("data", TString::kIgnoreCase) == 0){
    file = new TFile("/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/CMSSW_8X/rootfiles/mistag/h1btag/uhh2.AnalysisModuleRunner.DATA.DATA.root", "READ");
  } else if (sample.CompareTo("QCD", TString::kIgnoreCase) == 0){
    file = new TFile("/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/CMSSW_8X/rootfiles/mistag/h1btag/uhh2.AnalysisModuleRunner.MC.QCD.root", "READ");
  } 
  TH1F* d_all_old,*d_all,*d_trig_old,*d_trig;	
  ...
  if(obs=="pt"){
    d_all_old = (TH1F*) file->Get(name_all);
    d_all =  (TH1F*) d_all_old->Rebin(18,"d_all",bins);
  }else  d_all = (TH1F*) file->Get(name_all);
  d_all->Rebin(2);
  ...
  if(obs=="pt"){
     d_trig_old = (TH1F*) file->Get(name_trig);
     d_trig =  (TH1F*) d_trig_old->Rebin(18,"d_trig", bins);
  }else   d_trig = (TH1F*) file->Get(name_trig);
  d_trig->Rebin(2);
...
void draw_both_eff(TGraphAsymmErrors* eff, TString epsfilename, TString sample, double tot_eff, double tot_err,TGraphAsymmErrors* eff2, TString sample2, double tot_eff2, double tot_err2)
{
  ...
  painter->GetXaxis()->SetTitle(painter->GetTitle());
  painter->GetYaxis()->SetTitle("mistag rate H_{1b} tagger [%]");
  painter->SetTitle("");
  painter->GetXaxis()->SetTitleSize(0.05);
  painter->GetYaxis()->SetTitleSize(0.05);
  painter->GetXaxis()->SetLabelSize(0.04);
  ...
  painter->GetYaxis()->SetRangeUser(0, 0.05);
  // painter->GetXaxis()->SetRangeUser(-100, 1600);
  ...
  TPaveText* pt = new TPaveText(0.25,0.785,0.75,0.88,"nbNDC");
  pt->SetFillColor(kGray);
  pt->Draw();
  TString info = TString::Format("< #varepsilon_{MC} > = %4.3f #pm %4.5f", tot_eff, tot_err);
  TString info2 = TString::Format("< #varepsilon_{Data} > = %4.3f #pm %4.5f", tot_eff2, tot_err2);
  TLatex* text = new TLatex();
  text->SetTextFont(62);
  text->SetNDC();
  text->SetTextColor(eff->GetLineColor());
  text->SetTextSize(0.04);
  // text->DrawLatex(0.67, 0.2, info.Data());
  text->DrawLatex(0.27, 0.85, info.Data());
  text->SetTextColor(eff2->GetLineColor());
  text->DrawLatex(0.27, 0.8, info2.Data());
...
void draw_ratio(TGraphAsymmErrors* ratio, TString epsfilename, TString rname)
{
  ...
  if (epsfilename.Contains("isolation", TString::kIgnoreCase)){
    ...
    func = new TF1("Ratio", "[0]",170,  1400);
    //  func = new TF1("Ratio", "[0]",-1,  1);
    func->SetParName(0, "Ratio");
  }
  ratio->Fit(func, "R");
  
  if(epsfilename.Contains("pt.eps", TString::kIgnoreCase)){
    TFile *f= new TFile(unc_folder +"QCD_h1b.root","RECREATE");
