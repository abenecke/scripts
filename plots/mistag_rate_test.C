...
// TString folder = "/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/25ns/pictures/QCD/mass/mistagrate/"+unc_name+"/";
TString folder = "/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/25ns/pictures/QCD/mistagrate/test/"+unc_name+"/highpt_";
TString unc_folder = "/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/25ns/rootfile/QCD/mass/hists/lowpt/";
...
TGraphAsymmErrors* get_eff(TString sample, TString obs, TString hname1,double &tot_eff, double &tot_err,TString hname_all,TString hname_trig )
{
  ...
  //   cout<<"das is y "<<y <<"  das ist i "<< i <<"  das ist x "<< x<<endl;
  //  }
  // for(int i=3;i<44;i++){
  //   toteff->GetPoint(i, x, y);
  //   middle_y2 += y;
  ...
  // }
  // middle_y= middle_y/3;
  // middle_error_high = TMath::Sqrt(middle_error_high/3);
  // middle_error_low= TMath::Sqrt(middle_error_low/3);
  // middle_y2= middle_y2/(44-3);
  // middle_error_high2 = TMath::Sqrt(middle_error_high2/41);
  // middle_error_low2= TMath::Sqrt(middle_error_low2/41);
  ...
  // cout << "total eff (0-1400) = " <<middle_y  << " + " << middle_error_high << " - " <<middle_error_low  << endl;
...
void draw_both_eff(TGraphAsymmErrors* eff, TString epsfilename, TString sample, double tot_eff, double tot_err,TGraphAsymmErrors* eff2, TString sample2, double tot_eff2, double tot_err2)
{
  ...
  TString info = TString::Format("< #varepsilon > = %4.3f #pm %4.3f", tot_eff, tot_err);
  TString info2 = TString::Format("< #varepsilon > = %4.3f #pm %4.3f", tot_eff2, tot_err2);
  TLatex* text = new TLatex();
  text->SetTextFont(62);
  text->SetNDC();
  text->SetTextColor(eff->GetLineColor());
...
void draw_ratio(TGraphAsymmErrors* ratio, TString epsfilename, TString rname)
{
  ...
  if(epsfilename.Contains("pt.eps", TString::kIgnoreCase)){
    ...
    cout << "Hier" <<func->GetParameter(0)<< endl;
    tot_eff_h->Write();
    if(unc_name == "none"){
