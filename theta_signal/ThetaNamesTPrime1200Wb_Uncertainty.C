...
void ThetaNamesTPrime1200WB_Uncertainty()
{
  const bool top=true;
  const bool notop=true;
  const int nproc=9;

  TString process[nproc] = {"TTbar", "WJets","QCD","ST","DYJetsToLL","MC_ZPrime1500ToTPrime1200T_TPrimeToWB","MC_ZPrime2000ToTPrime1200T_TPrimeToWB","MC_ZPrime2500ToTPrime1200T_TPrimeToWB","DATA"};
 ...
 for(int m=0; m<2; m++){
  ...
  for(int j=0; j<Nsyst; ++j){
    ...
    for(int i=0; i<nmax; ++i){
      ...
      TH1F* M_higgstop_old = (TH1F*)infile->Get("higgs_top_chi2min_btag50/M_ZPrime_rec");
      ...
      TH1F* M_higgsnotop_old = (TH1F*)infile->Get("higgs_notop_chi2min_btag50/M_ZPrime_rec");
      ...
      // M_zwtop->Rebin(2);
       TH1F* M_zwtop_old = (TH1F*)infile->Get("zw_top_chi2min_btag50/M_ZPrime_rec");
	...
	M_higgstop->SetName("MZPrime_TPrime1200WB_higgstop__"+process[i]+"__"+systname[j]+"__"+systshift2);
	M_higgsnotop->SetName("MZPrime_TPrime1200WB_higgsnotop__"+process[i]+"__"+systname[j]+"__"+systshift2);
	M_zwtop->SetName("MZPrime_TPrime1200WB_zwtop__"+process[i]+"__"+systname[j]+"__"+systshift2);
	M_zwnotop->SetName("MZPrime_TPrime1200WB_zwnotop__"+process[i]+"__"+systname[j]+"__"+systshift2);
 
      }
      else{
	cout <<"Set name for default"<< endl;
	M_higgstop ->SetName("MZPrime_TPrime1200WB_higgstop__"+process[i]);
	M_higgsnotop->SetName("MZPrime_TPrime1200WB_higgsnotop__"+process[i]);
	M_zwtop->SetName("MZPrime_TPrime1200WB_zwtop__"+process[i]);
	M_zwnotop->SetName("MZPrime_TPrime1200WB_zwnotop__"+process[i]);
