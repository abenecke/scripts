...
void ThetaNamesTPrime1200WB()
{
  const bool top=true;
  const bool notop=true;
  const int nproc=8;

  TString process[nproc] = {"TTbar", "WJets","QCD","ST","DYJetsToLL","MC_ZPrime1500ToTPrime1200T_TPrimeToWB","MC_ZPrime2000ToTPrime1200T_TPrimeToWB","MC_ZPrime2500ToTPrime1200T_TPrimeToWB"};
  ...
  for(int j=0; j<Nsyst; ++j){
    ...
    TString outfilename = "/nfs/dust/cms/user/abenecke/ZPrimeTotTPrime/25ns/rootfile/chi2cut/theta_histograms_MZPrime";
    if(systname[j]!="default"){
