...
void ThetaNamesTPrime900HT_Uncertainty()
{
  const bool top=true;
  const bool notop=true;
  const int nproc=8;

  TString process[nproc] = {"TTbar", "WJets","QCD","ST","DYJetsToLL","MC_ZPrime1500ToTPrime900T_TPrimeToHT","MC_ZPrime2000ToTPrime900T_TPrimeToHT","DATA"};
 ...
 for(int m=0; m<2; m++){
  ...
  for(int j=0; j<Nsyst; ++j){
	...
	M_higgstop->SetName("MZPrime_TPrime900HT_higgstop__"+process[i]+"__"+systname[j]+"__"+systshift2);
	M_higgsnotop->SetName("MZPrime_TPrime900HT_higgsnotop__"+process[i]+"__"+systname[j]+"__"+systshift2);
	M_zwtop->SetName("MZPrime_TPrime900HT_zwtop__"+process[i]+"__"+systname[j]+"__"+systshift2);
	M_zwnotop->SetName("MZPrime_TPrime900HT_zwnotop__"+process[i]+"__"+systname[j]+"__"+systshift2);
 
      }
      else{
	cout <<"Set name for default"<< endl;
	M_higgstop ->SetName("MZPrime_TPrime900HT_higgstop__"+process[i]);
	M_higgsnotop->SetName("MZPrime_TPrime900HT_higgsnotop__"+process[i]);
	M_zwtop->SetName("MZPrime_TPrime900HT_zwtop__"+process[i]);
	M_zwnotop->SetName("MZPrime_TPrime900HT_zwnotop__"+process[i]);
